...
def sample_object():
    ...
    obj = SHAPES[shape]()
    rgb = COLORS[color] + 0.5 * (random.random(size=3) - 0.5)
    rgb = np.clip(rgb, 0, 1)
    obj.visual.vertex_colors = rgb
...
def sample_coords(cx=0, cy=0, cz=0):
    return (
        cx + np.random.random() * 0.05,
        cy + np.random.random() * 0.05,
        cz + np.random.random() * 0.05,
    )

def sample_scene():
    scene = pyrender.Scene()
    obj1_desc, obj1 = sample_object()
    p1x, p1y, p1z = sample_coords(0, -.15, 0)
    #p1x, p1y, p1z = 0, -.15, 0
    ...
    if random.random() < 0.5:
        ...
        p2x, p2y, p2z = sample_coords(0, .15, 0)
        #p2x, p2y, p2z = 0, .15, 0
